...
export const getArmyPda = async (keyPair) => {
...
export const createArmyAccount = async (connection, keyPair) => {
    ...
    const armyPDA = await getArmyPda(keyPair);
...
export const getArmyData = async (connection, keyPair) => {
    const [armyPDA] = PublicKey.findProgramAddressSync(
        [Buffer.from("ARMY"), keyPair.publicKey.toBuffer()],
        programId
    );
...
export const showArmyData = async (connection, keyPair) => {
    ...
    const armyData = await getArmyData(connection, keyPair);
    if (!armyData) {
        console.log("Army account not found. It may not have been created yet.");
        return;
    }
    let { owner, zombies } = armyData;
    console.log("Army owner:", owner.toBase58());
    console.log("Zombies:");
    zombies.forEach((zombie, i) => {
        console.log(`  Zombie ${i + 1}:`);
        console.log(`    DNA: ${zombie.dna}`);
        console.log(`    Last Fight: ${new Date(Number(zombie.lastFight) * 1000)}`);
        console.log(`    XP: ${zombie.xp}`);
    })
}

export const removeZombie = async ( connection, armyPDA, keyPair, zombieId) => {
    const blockhashInfo = await connection.getLatestBlockhash();
...
export const initiateBattle = async (
    ...
    try {
        ...
        const battleData = await getBattleData(connection, txHash);
        return { battleData, battlePDA}
    } catch (error) {
        // console.error("Error initiating battle:", error);
        throw error;
    }
...
export const getBattleData = async (connection, txHash) => {
    try {
        ...
        return { owner, shuffledOrder, zombieId, selection, outcome };
    } catch (error) {
...
export const showBattleData = async (connection, txHash) => {
    ...
    let { owner, shuffledOrder, zombieId, selection, outcome } = battleData;
